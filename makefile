build:
	...
	cd /Windows-Deployer-KVM-Web && npm install && npm run build
	mv /Windows-Deployer-KVM-Web/build /app/static
	cd /app && go get -u github.com/gobuffalo/packr/packr && export GOPATH=$HOME/go && export PATH=$PATH:$GOROOT/bin:$GOPATH/bin && packr
